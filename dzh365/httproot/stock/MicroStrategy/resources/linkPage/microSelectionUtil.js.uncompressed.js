...
define("stock/MicroStrategy/resources/linkPage/microSelectionUtil", [ "dijit/registry", "dojo/_base/config",
	...
	_util.addVotes = function(userId,microCode,nodeId,thisWeekDivId,fn){
		...
		console.log(config.dzhURL+"/MicroStrategyBG/microChoose/microVotes?"+ioQuery.objectToQuery(parameter))
		myGrid.renderQuery({
			service:'rpc',
			so:'javadataproxy.jar',
			'function':'gw/UIMain',
	...
	_util.startup = function(){ 
		...
		function formatButtom002(rowIndex, colIndex, value, cellObj, rowObj, gridObj){
			...
			console.log(rowObj.voteFlag+","+rowObj.userVotes)
		...
		console.log(config.dzhURL+"/MicroStrategyBG/microChoose/getTop20?"+ioQuery.objectToQuery(parameter))
		myGrid.renderQuery({
			service:'rpc',
			so:'javadataproxy.jar',
			'function':'gw/UIMain',
