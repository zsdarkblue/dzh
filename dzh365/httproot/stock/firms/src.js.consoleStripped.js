	...
	return declare("dojox.websocket.WebsocketFileReader", [_SocketBase], {
		...
		read : function (file, convertUrl) {
			...
			var deferred = new Deferred(function(reason){
			   0 && console.log("cancel");
			});
			
			this.deferreds.push(deferred);
			0 && console.log(new Date().getTime(), "开始读取文件 " + file );
			this.send(file);
		...
		_onmessage : function ( evt ) {
			...
			//0 && console.dir(evt);
			//0 && console.log(evt.data);
			var detail = evt.data;
			0 && console.log(new Date().getTime(), "读取文件返回，文件严格按照读取顺序返回");
...
define("dojox/websocket/_base", ["dojo/_base/lang", "dojo/_base/declare", "dojo/Deferred"], function (lang, declare, Deferred) {
	...
	return declare("dojox/websocket/_base", null, {
		...
		close : function () {
			...
			0 && console.log("closed the socket in _base");
		},
		_onopen : function(evt){
			while(this.send_queue.length >0){
				...
				try {
					...
					0 && console.log(err.description);
				}
			}
		},
		
		_onmessage : function(evt){
			0 && console.log("_base log : ", evt.data);
		},
